...
class BaseGraph(Data):
    def __init__(self, x, edge_index, edge_weight, subG_node, subG_label,
        ...
        Args:
            ...
            skip_undirected: if True, skip the to_undirected() call in __init__.
                Required for Elliptic2 (383M edges) where the check+rebuild
                would OOM. The preprocessing pipeline already makes edges
                undirected, so this is safe for elliptic2.
        '''
...
def load_dataset(name: str):
    ...
    if name in ["coreness", "cut_ratio", "density", "component"]:
        ...
        # Loads the k=2 induced subgraph prepared by preprocess_elliptic2.py.
        # File: dataset_/elliptic2/processed/elliptic2_k2.pt
        # Contents (see preprocess_elliptic2.py for exact shapes):
        #   x_features  (N_induced, 43)   float32  -- real-valued node features
        #   edge_index  (2, E_induced)    long     -- already undirected
        #   edge_weight (E_induced,)      float32  -- all ones
        #   pos         (110902, 547)     long     -- padded subG node matrix
        #   y           (110902,)         float32  -- binary labels (42:1 imbalance)
        #   mask        (110902,)         long     -- 0=train,1=val,2=test
        #   num_nodes, k_hops, global_ids
        path = "./dataset_/elliptic2/processed/elliptic2_k2.pt"
        print(f"[elliptic2] loading {path}")
        blob = torch.load(path, map_location="cpu", weights_only=False)
        x_feat = blob["x_features"]  # (N, 43) float32
        # BaseGraph / GLASS NodeEmb expect x of shape (N, K, D) with K=1 view.
        x = x_feat.unsqueeze(1)  # (N, 1, 43)
        edge_index = blob["edge_index"].long()
        edge_weight = blob["edge_weight"].float()
        pos = blob["pos"].long()
        y = blob["y"].float().reshape(-1)
        mask = blob["mask"].long()
        print(
            f"[elliptic2] x={tuple(x.shape)} ei={tuple(edge_index.shape)} "
            f"pos={tuple(pos.shape)} y={tuple(y.shape)} "
            f"pos_count={int(y.sum())} neg_count={int((y == 0).sum())}"
        )
        return BaseGraph(x,
                         edge_index,
                         edge_weight,
